...
// Bilanciamento ruote "vecchio metodo" — non più usato ora che c'è il PID,
// lasciato solo come riferimento storico
// const float BALANCE_L = 127.0f / 144.0f;  // ≈ 0.882
// const float BALANCE_R = 1.0f;
...
// ───────────────────────── PID trazione (NUOVO) ─────────────
// Target di velocità impostati dal comando "V" ricevuto da ROS
float target_v_l = 0.0f, target_v_r = 0.0f;   // [m/s]

// Velocità misurate dagli encoder, aggiornate ogni PID_PERIOD (per debug/telemetria)
float measured_v_l = 0.0f, measured_v_r = 0.0f;

// Guadagni PID — DA TARARE sperimentalmente, questi sono solo valori di partenza
float Kp_pid = 800.0f;
...
const unsigned long PID_PERIOD = 30;   // ms, più veloce di ODO_PERIOD per un controllo reattivo
...
const unsigned long PID_DEBUG_PERIOD = 500;   // stampa target/misurata ogni 500ms
...
void handleROS() {
  if (Serial.available()) {
    ...
    if (line.startsWith("V")) {
      ...
      if (sp > 0) {
        ...
        // ora impostiamo solo i target: ci pensa il PID a muovere i motori
        target_v_r = v_lin + v_ang * TRACK_BASE / 2.0f; //  cambiato poichè girava a destra quando doveva girare a sinistra e viceversa
        target_v_l = v_lin - v_ang * TRACK_BASE / 2.0f;
      }
    }
  ...
  if (millis() - lastOdoUpdate >= ODO_PERIOD) {
    ...
    // ── Diagnostica: encoder fermi da troppo tempo ──
...
void updateTractionPID() {
  ...
  // stessa convenzione di segno usata in updateOdometryCalc (DX invertito)
  ...
  // ── Debug: target vs misurata, ogni 500ms ──
...
float computePID(float target, float measured, float &integral, float &lastError, float dt) {
  // robot fermo: azzera tutto, niente lotta o deriva del motore
  ...
  // feedforward: stessa scala usata prima (0.5 m/s -> PWM 255), il PID corregge l'errore residuo

...
// Guadagni PID — modificabili a runtime dal menu LCD
float Kp_pid = 800.0f;
...
// ───────────────────────── Menu e Interfaccia ───────────────
typedef enum { MENU_NAV = 0, SUB_SPAZ, SUB_GYRO, SUB_PID } MenuState;
MenuState statoMenu = MENU_NAV;

const uint8_t MENU_ITEMS = 3;   // Spazzola, Giroscopio, PID Gains
uint8_t  menuSel    = 0;
...
// ═══════════════════════════════════════════════════════════════
//  Step di incremento per ciascun guadagno PID
// ═══════════════════════════════════════════════════════════════
...
void handleMenu() {
    ...
    case SUB_PID:
      ...
      if (tasto) {
        if (pidField == 3) {
          ...
          // entra/esce dalla modalità di modifica valore
  ...
  if (aggiornaSchermo) {
    switch (statoMenu) {
      ...
      case MENU_NAV: {
        const char* names[MENU_ITEMS] = { "Spazzola", "Giroscopio", "PID Gains" };
